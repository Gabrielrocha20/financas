...
const AddEntryModal = ({ visible, onClose, onAddEntry, paymentMethods }) => {
  const [paymentMethod, setPaymentMethod] = useState('');
  const [entryName, setEntryName] = useState('');
  const [entryValue, setEntryValue] = useState('');

  const handleAddEntry = () => {
    // Verifica se o meio de pagamento foi selecionado
    if (!paymentMethod) {
    ...
    const newEntry = {
      label: entryName,
      value: entryValue,
      data: getCurrentDate(), // Use a data selecionada pelo usuário
    ...
    >
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>Adicionar Nova Entrada</Text>
          <RNPickerSelect
